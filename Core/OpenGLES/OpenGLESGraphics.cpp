...
//		glBegin(GL_QUADS);
		for (int i = 0; i < nodes.size()-1; i++)
		{
			if (nodes[i]->variant != -1 && nodes[i]->variant < cells)
			{
                ...
                GLfloat vertex_arr[8] = {
                    pVectorVertex[0].x, pVectorVertex[0].y,
                    pVectorVertex[1].x, pVectorVertex[1].y,
                    pVectorVertex[2].x, pVectorVertex[2].y,
                    pVectorVertex[3].x, pVectorVertex[3].y
                };

...
//		}
	...
	Puppet::Puppet()
		: Entity(), isPlaying(false), isPaused(false), textureAtlas(NULL), currentAnimation(NULL)
	{
	}
	...
	void Puppet::Load(const std::string &filename)
	{
	...
	void Puppet::Added()
	{
		for (std::list<Part*>::iterator i = this->parts.begin(); i != this->parts.end(); ++i)
		{
			Part *iterPart = (*i);
			if (iterPart)
			{
